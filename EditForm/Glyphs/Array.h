...
public:
	...
	Long RowarSearchUnique(void* key, int(*compare)(void*, void*));
	void RowarSearchDuplicate(void* key, Long* (*indexse), Long* count, int(*compare)(void*, void*));
...
/*
명칭 : RowarSearchUnique
기능 : 배열에서 선형검색을 한다.
입력 : 키
출력 : 위치
*/
template <typename T>
Long Array<T>::RowarSearchUnique(void* key, int(*compare)(void*, void*)) {
	Long index = -1;
	Long i = 0;
...
/*
명칭 : RowarSearchDuplicate
기능 : 배열에서 중복 선형검색을 한다.
입력 : 키
출력 : 위치들, 개수
*/
template <typename T>
void Array<T>::RowarSearchDuplicate(void* key, Long* (*indexes), Long* count, int(*compare)(void*, void*)) {
	Long i = 0;
	Long j = 0;
	*count = 0;
